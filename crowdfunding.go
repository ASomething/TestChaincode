...
// Define the Event structure. Structure tags are used by encoding/json library       --- 一个event对应多个Donations
type Event struct {
...
// Define the Event structure. Structure tags are used by encoding/json library       --- 一个donation对应一个user
...
// Define the Event structure. Structure tags are used by encoding/json library       --- 一个用户对应一个email
type User struct {
	UserID        string `json:"user_id"`
...
func (s *SmartContract) queryAllDonations(APIstub shim.ChaincodeStubInterface) sc.Response {
	...
	eventAsBytes, err := APIstub.GetState("E1")         // 既然这样，就表明这里的“event”只有“E1”了
...
func (s *SmartContract) donateMoney(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	donatedQuantity, _ = strconv.Atoi(args[0])          // 此次捐款金额
	...
	existingVal = event.DonatedAmount                 // 此次捐款之前的总金额
	newVal = existingVal + donatedQuantity            // 捐款之后更新的总金额
	if newVal > event.RequiredAmount {
		return shim.Error("Donated quantity is more than required. Hence could not donate.")
	} else {
		event.DonatedAmount = existingVal + donatedQuantity        // newVal
		event.DonationIDs = append(event.DonationIDs, donationID)
		eventAsBytes, _ := json.Marshal(event)
		APIstub.PutState("E1", eventAsBytes)            // 1
		...
		APIstub.PutState(donationID, donateAsBytes)    // 2
		...
		APIstub.PutState(userID, userAsBytes)          // 3
		return shim.Success(nil)
	}
}
